...
class Partials extends Collection {
  ...
  objectRelativeTo(file) {
    ...
    for (const partial of Object.keys(this.orphans)) {
      let relPath = up.relative(file.dir, partial);
      const ext = up.extname(relPath);
      if (up.extname(relPath) === '.mustache') {
        relPath = up.trimExt(relPath, [], ext.length);
      }
      const ext2 = up.extname(relPath);
      if (up.extname(relPath) === '.partial') {
        relPath = up.trimExt(relPath, [], ext2.length);
      }
      orphans[relPath] = this.orphans[partial].content;
    }
    if (this.dirty) {
      this.namedPartials = {};
      for (const partial of Object.keys(this.insideRoot)) {
        ...
        const ext2 = up.extname(relPath);
        if (up.extname(relPath) === '.relPath') {
          relPath = up.trimExt(relPath, [], ext2.length);
        }
        this.namedPartials[relPath] = this.insideRoot[partial].content;
      }
      this.dirty = false;
    }
